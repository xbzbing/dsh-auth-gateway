...
The gateway injects a compatibility bootstrap via `webServer.tapIndex`, after the DSH queue-mode `__ModuleLoader__` is created and before parser-preloaded bundles register. The bootstrap only wraps `@deepseek-ai/dsh-client-connection` and sets `handle.isLoopback = true` before its synchronous `ctx.provide('connection', handle)` publishes the service; authentication, HTTP/WS interception, Host/Origin rewriting and the server-side privileged fence all stay untouched. External pages still must pass the full session gate (incl. onboarding/OTP); the internal DSH must keep listening only on `127.0.0.1`. If the loader protocol changes, the gateway logs a compatibility warning without disabling any security check.
