...
export class LoginGateway {
  ...
  async #route(req, res) {
    ...
    // could shadow a future internal method.
    if (req.method === 'GET' && pathname === '/login-api/settings') {
      return this.#handleGetSettings(req, res)
    }
    if (req.method === 'POST' && pathname === '/login-api/settings') {
  ...
  #verifiedTokenOr401(req, res) {
    ...
    const otpActive = this.otp.otpEnabled && hasOTP() && getOTPStatus().enabled
  ...
  /** POST /login-api/settings — update plugin configuration. */
