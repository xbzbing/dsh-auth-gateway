...
`dsh-auth-gateway` 本身是一个独立监听的网关进程（运行在 dsh 进程内），可以直接对外提供服务，也可以放在 nginx 反向代理之后。本文覆盖三种典型拓扑，均给出完整配置示例。
...
```
拓扑 A：裸金属直连          拓扑 B：宿主机 nginx         拓扑 C：Docker nginx 容器
浏览器 ──> 网关:8080        浏览器 ──> nginx:443           浏览器 ──> nginx:443
                                   │                            │
                                   └─> 网关:8080 (宿主机)       └─> 网关:8080 (宿主机)
                                                          (nginx 在容器里，经
                                                           host-gateway 访问宿主机)
```

三种拓扑中，网关侧的配置完全一致（`listenHost: 0.0.0.0`），区别只在"谁在前面"以及路径前缀的配置。
...
## 拓扑 B：宿主机 nginx + 网关（根路径部署）

**适用**：单机、nginx 与 dsh 同宿主，希望用 443 + 域名 + HTTPS 对外。
...
## 拓扑 C：宿主机 nginx + 网关（子路径部署，如 `/dsh/`）

**适用**：同一域名下还跑着其他 Web 应用，dsh 需要挂在子路径（如 `https://example.com/dsh/`）。
...
**适用**：nginx 跑在 Docker 容器里，dsh 网关直接跑在宿主机（或另一个容器）。nginx 容器需要一种方式访问宿主机端口。
...
子路径部署（`/dsh/`）时，把拓扑 C 的 location 写法搬过来，`proxy_pass` 目标换成 `http://host.docker.internal:8080`（或 `/` 形式去前缀）即可。
...
- [DEPLOYMENT.md](DEPLOYMENT.md) — 端口推导、LAN 部署、故障排查
