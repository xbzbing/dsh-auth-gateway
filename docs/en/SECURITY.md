...
- **Outbound network request (new-version check)**: this is the plugin's **only** outbound connection. When the settings panel opens its About card, the gateway issues one GET to the public npm registry (`https://registry.npmjs.org/dsh-auth-gateway/latest`, 3s timeout) to see whether a newer release exists; the request carries no credentials, session cookie or instance identifier, but your deployment's egress IP is visible to the registry. Results and failures are cached in memory (6h success, 15min failure); an unreachable registry makes the panel say "update check unavailable" rather than error out, and neither the auth nor the forwarding path is affected. For a fully offline setup, set `updateCheck: false` — the panel still shows the running version and the repository link, both read from the local `package.json` with no network involved.
