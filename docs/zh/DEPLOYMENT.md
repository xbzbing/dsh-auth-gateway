...
- **混合 HTTPS/HTTP 入口（双协议同域）**：会话 Cookie 带 `SameSite=Strict`，且 HTTPS 登录的 Cookie 带 `Secure`——浏览器因此不会在 `http://` 入口携带 HTTPS 入口的会话：`https://dsh.example.com` 登录后访问 `http://dsh.example.com:8080` 会需要重新登录。这是**预期行为**（加密入口的会话不降级到明文入口）；若确实需要多入口共享会话，请让所有入口统一走 HTTPS（同一反代上各端口都部署 TLS），而不是放宽 cookie 属性。
