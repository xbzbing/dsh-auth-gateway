 ...
 *   housekeeping failures during rotation (rename, prune) are likewise
 *   isolated — the append proceeds into whatever live file exists;
 ...
 * - writes go through fs.appendFile (open-write-close per line), so a rename
 *   during rotation can never leave a stale fd writing into an archived file;
...
import { chmod, mkdir, readdir, rename, appendFile, stat, unlink } from 'node:fs/promises'
...
export class AuditLogWriter {
  ...
  async #append(event) {
    ...
    if (day !== this.currentDay) await this.#rollTo(day)
   ...
   * the stale file from its mtime.
   *
   * Housekeeping failures (rename, prune) are caught and routed to onError so
   * the append that triggered the rotation is never lost — the event lands in
   * whatever live file exists.
   */
  async #rollTo(day) {
    const live = join(this.dir, AUDIT_LOG_NAME)
    let stamp = this.currentDay
    if (stamp === undefined) {
      try {
    ...
    if (stamp !== undefined && stamp !== day) {
      ...
      // Clock stepped backwards after an earlier rotation: never overwrite.
      for (let n = 2; existsSync(dest); n++) dest = join(this.dir, `${AUDIT_LOG_NAME}.${stamp}-${n}`)
      try {
        await rename(live, dest)
      } catch (err) {
        // Rotation failed — the live file still holds yesterday's content.
        // Append into it anyway; a future rotation will retry the archive.
        this.#fail(err)
      }
    }
    this.currentDay = day
