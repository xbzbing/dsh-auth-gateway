    ...
    try {
      if (reset) {
        ...
        return json(res, 200, {
          ...
          // Reissue the caller's own session cookie under the restored
          // policy, so THIS browser's stored cookie matches it immediately
          // instead of keeping the old attribute until its next login.
        }, sessionCookie(token, secureForPolicy(cookieSecureMode(), req)))
      }
      ...
      return json(res, 200, {
        ...
        // Same reissue as reset: the new policy takes effect on the very
        // next request, not on the next login. The browser accepts the
        // overwrite only from a SECURE origin (a plaintext entry cannot
        // replace a stored Secure cookie — the panel warns for that case).
      }, sessionCookie(token, secureForPolicy(mode, req)))
    } catch (err) {
