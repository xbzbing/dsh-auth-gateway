...
export class LoginGateway {
  ...
  async #route(req, res) {
    ...
    const otpEnabled = hasOTP() && getOTPStatus().enabled
  ...
  async #handleAuth(req, res) {
    ...
    if (otpRequired && otpEnabled) {
      // Password verified, but OTP required - don't issue session yet
      // Client should redirect to /otp/verify
  ...
  async #handleOTPDisable(req, res) {
    ...
    // If OTP is enabled, require OTP verification to disable
    if (hasOTP() && getOTPStatus().enabled) {
      if (typeof otp !== 'string' || otp.length !== (this.otp.otpDigits || 6)) {
