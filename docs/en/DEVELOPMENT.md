...
- **Settings nav order**: a `settings.section` registration's `order` must be **strictly greater than every shipped section's maximum** (dsh currently ships `general` 0 / `models` 10 / `plugins` 15 / `agent-presets` 20; this plugin uses 100, dsh's own example value for a contributed entry). The slot list sorts by `order` with a stable sort, so a value equal to a shipped one degrades to plugin load order and drifts across "Agent Presets" depending on the composition; `tests/client-contract.test.mjs` asserts this invariant;
