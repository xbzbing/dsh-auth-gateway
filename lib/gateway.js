...
import { SessionStore, sessionCookie, expiredCookie, tokenFromCookieHeader, requestIsSecure, secureForPolicy } from './auth.js'
import { setPassword, verifyPassword, isInitialPassword } from './store.js'
...
import { createRateLimiter, retryAfterSeconds } from './rate-limit.js'
...
export class LoginGateway {
   ...
   *   plugin entry from ctx.credentials).
   ...
   */
  constructor({ listenHost, listenPort, upstreamHost, upstreamPort, basePath = '/', policy = {}, otp = {}, cookieSecure = 'auto', cookieSecureOverride, cookieSecureOverrideStore, upstreamSecretReader, localePreference = () => undefined, versionInfo, updateChecker, updateCheckAuto = false }) {
  ...
  /**
   * Every route the gateway answers itself: [method, pattern, handler], with
   * a trailing `*` marking a prefix match. Keep the `/` in the pattern
   * (`/assets/*`): the marker matches verbatim, so that spelling covers whole
   * path segments only, while a hypothetical `/otp*` would also swallow
   * `/otpX`. Anything not listed falls through to the auth gate in #route.
   * Built once — the handlers close over `this`.
   *
   * The comments are the routing security rationale (why a path lives under
   * this prefix, what may reach it unauthenticated), not decoration.
   *
   * @type {Array<[string, string, (req: http.IncomingMessage, res: http.ServerResponse) => void]>}
   */
  #routes = [
    // Auth API + login page are served directly by the gateway.
    ['GET', '/login', (req, res) => this.#serveLoginPage(req, res)],
    ['POST', '/login/auth', (req, res) => this.#handleAuth(req, res)],
    ['POST', '/login/change', (req, res) => this.#handleChange(req, res)],
    ['POST', '/login/logout', (req, res) => this.#handleLogout(req, res)],
    ...
    ['GET', '/login-api/settings', (req, res) => this.panelApi.serveSettings(req, res)],
    ...
    ['GET', '/login-api/session', (req, res) => this.panelApi.serveSessionProbe(req, res)],
    ['POST', '/login-api/cookie-secure', (req, res) => this.panelApi.handleSetCookieSecure(req, res)],
    ...
    ['GET', '/login-api/version', (req, res) => this.panelApi.handleGetVersion(req, res)],

    // OTP routes (handlers in gateway-otp.js)
    ['GET', '/otp/setup', (req, res) => this.otpRoutes.serveSetupPage(req, res)],
    ['POST', '/otp/enable', (req, res) => this.otpRoutes.handleEnable(req, res)],
    ['POST', '/otp/verify-setup', (req, res) => this.otpRoutes.handleVerifySetup(req, res)],
    ['GET', '/otp/verify', (req, res) => this.otpRoutes.serveVerifyPage(req, res)],
    ['POST', '/otp/verify', (req, res) => this.otpRoutes.handleVerify(req, res)],
    ['POST', '/otp/verify-backup', (req, res) => this.otpRoutes.handleVerifyBackup(req, res)],
    ['POST', '/otp/disable', (req, res) => this.otpRoutes.handleDisable(req, res)],

    ['GET', '/onboarding', (req, res) => this.#serveOnboardingPage(req, res)],
    ['GET', '/onboarding/password', (req, res) => this.#serveOnboardingPasswordPage(req, res)],
    ...
    // 直接出静态文件，此处属纵深防御）。
    ['GET', '/manifest.webmanifest', (req, res) => this.#forward(req, res, { upstreamAuth: false })],
    ['GET', '/favicon.svg', (req, res) => this.#forward(req, res, { upstreamAuth: false })],
    ['GET', '/assets/*', (req, res) => this.#forward(req, res, { upstreamAuth: false })],
  ]
  ...
  async #route(req, res) {
    ...
    // Strip the basePath prefix so internal route matching stays
    // root-relative; a path outside it (e.g. '/dsh2/foo' with basePath
    // '/dsh') keeps its raw spelling.
    const pathname = this.#stripPathPrefix(rawPathname) ?? rawPathname
    ...
    if (!this.sessions.isValid(token)) {
      return this.#refuse(res, pathname, 'unauthenticated', '/login')
    }
    ...
    if (this.sessions.isNeedsOnboarding(token)) {
      return this.#refuse(res, pathname, 'onboarding-required', '/onboarding')
    }

    // Third gate: OTP is active but this session has not verified it yet.
    if (this.#otpActive() && !this.sessions.isOTPVerified(token)) {
      return this.#refuse(res, pathname, 'otp-required', '/otp/verify')
    }
  ...
  /**
   * 302 to a gateway page; `location` is root-relative and rides the
   * basePath. Carries `no-store` like every other auth response: a cached
   * gate redirect bounces a signed-in user back to /login (or the reverse)
   * long after the session changed.
   */
  #redirect(res, location) {
    res.writeHead(302, { location: this.basePath + location, 'cache-control': 'no-store' })
    res.end()
  }

  /**
   * Refuse a request that failed a session gate: `/api/*` answers 401 JSON
   * (a redirect would break client code that expects data), every other
   * path a 302 to `location`.
   * @param {string} pathname - basePath-stripped request path.
   * @param {string} error - machine code for the JSON answer.
   * @param {string} location - root-relative page to redirect to.
   */
  #refuse(res, pathname, error, location) {
    if (isApiPath(pathname)) {
      // Through #json, not a bespoke writeHead: this payload describes
      // security state (unauthenticated / onboarding-required /
      // otp-required), so it must carry no-store like every other auth
      // answer — and the headers stay defined in exactly one place.
      this.#json(res, 401, { ok: false, error })
    } else {
      this.#redirect(res, location)
    }
  }

  /**
   * Gate a page that only makes sense behind a live session: answers the
   * /login redirect itself and returns false when the cookie names no live
   * session.
   */
  #requireSession(req, res) {
    if (this.sessions.isValid(tokenFromCookieHeader(req.headers.cookie))) return true
   ...
   */
  #pageLocale(req) {
    return pageLocale(this.localePreference(), req.headers['accept-language'])
  }
  ...
  #serveOnboardingPage(req, res) {
    ...
    if (this.#otpActive()) {
      this.#redirect(res, '/onboarding/password')
      return
    }
    sendHtml(res, onboardingPageHtml({
  ...
  #serveOnboardingPasswordPage(req, res) {
    if (!this.#requireSession(req, res)) return
    sendHtml(res, onboardingPasswordPageHtml({
      locale: this.#pageLocale(req),
      basePath: this.basePath,
    }))
  }
  ...
  async #handleAuth(req, res) {
    ...
    // The maps are shared with gateway-otp, whose addressOf(req) must spell
    // this key identically or lockout and OTP-window counts would split.
    const key = authIp
    ...
    if (entry !== undefined) {
      if (entry.lockedUntil > now) {
        ...
        const retryAfter = retryAfterSeconds(entry.lockedUntil, now)
    ...
    // 2FA is required at login whenever a binding is active.
    const otpEnabled = this.#otpActive()
  ...
  /**
   * Whether 2FA is active: a stored secret exists and the record says
   * enabled.
   */
  #otpActive() {
  ...
  #verifyOtp(secret, otp, address) {
    ...
    const result = verifyTOTP(secret, otp, {
      // `?? 1`, never `|| 1`: the config allows a zero-width window (0 = no
      // tolerance) and `||` would silently coerce it to 1.
  ...
  async #handleChange(req, res) {
    ...
    if (!await verifyPassword(oldPassword)) {
      ...
      // Same key spelling as gateway-otp's addressOf(req): one address, one
      // shared lockout counter across endpoints.
      const key = changeIp
  ...
  /** JSON response with an optional Set-Cookie header. Auth responses carry
   *  no-store: the payloads describe security state (OTP status, cookie
   *  policy) and must never be served from a shared cache. */
   ...
   * the request's transport. The decision itself lives in auth.js
   * (secureForPolicy), shared with the panel's post-save cookie reissue.
   */
  #secureCookie(req, mode = this.panelApi.cookieSecureMode()) {
    return secureForPolicy(mode, req)
  }
  ...
  #secureCookieOrWarn(req) {
    ...
    const secure = this.#secureCookie(req, mode)
    ...
    if (mode === true && !requestIsSecure(req)) {
  ...
  #stripBasePath(rawUrl) {
    ...
    try {
      ...
      const stripped = this.#stripPathPrefix(url.pathname)
      if (stripped === null) return rawUrl
      return stripped + url.search
    } catch {
      return rawUrl // unparsable target: forward it verbatim
    }
  }

  /**
   * Strip the basePath prefix from a root-relative pathname, boundary-checked
   * so '/dsh2/foo' is NOT treated as basePath '/dsh' + '/2/foo'. Returns the
   * remainder ('/' when the path is exactly the basePath), or null when the
   * path lies outside the basePath.
   * @param {string} pathname
   * @returns {string|null}
   */
  #stripPathPrefix(pathname) {
...
/**
 * Whether a (basePath-stripped) pathname is inside dsh's /api namespace —
 * answered 401 instead of redirected. Boundary-checked: '/api' and '/api/…'
 * qualify; a lookalike like '/apiff' does not (it is a redirect-only path,
 * still refused — the check only picks the response shape).
 */
 ...
 */
export function createGateway(config, { upstreamSecretReader, localePreference, versionInfo, updateChecker, updateCheckAuto, cookieSecureOverride, cookieSecureOverrideStore } = {}) {
  ...
  return new LoginGateway({ listenHost, listenPort, upstreamHost, upstreamPort, basePath, policy, otp, cookieSecure: config?.cookieSecure ?? 'auto', cookieSecureOverride, cookieSecureOverrideStore, upstreamSecretReader, localePreference, versionInfo, updateChecker, updateCheckAuto })
}
