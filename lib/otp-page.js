...
const page = (locale) => makePage(STYLE, scriptHead(errorsFor(locale, ERROR_KEYS, ERROR_OVERRIDES)))
...
document.getElementById('verify-form').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    if (done) {
      ...
      if (data.next === '/onboarding/password') done.textContent = '${str.btnNext}';
      done.onclick = () => { location.href = data.next || '/'; };
    }
    err.textContent = '';
  } catch (ex) {
...
<div class="step">
  ...
  <div id="setup-done" style="display:none; margin-top:12px;">
    ...
    <button type="button" id="setup-done-button" onclick="location.href='/'">${str.btnDone}</button>
  </div>
</div>
