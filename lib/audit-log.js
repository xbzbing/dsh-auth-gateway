...
/** Bound in-memory backlog from high-rate fire-and-forget audit events. */
export const MAX_PENDING_WRITES = 1024

/** Per-day audit-file cap; prevents a hostile request stream filling a disk. */
export const MAX_LIVE_BYTES = 1024 * 1024
...
export class AuditLogWriter {
   ...
   * @param {string} [options.legacyDir] - directory the pre-0.5.1 writer used
   *   (default: `dir`'s parent, i.e. the credential dir for the default
   *   `dir`). Explicit so an injected `dir` cannot silently aim the startup
   *   migration at an unrelated tree.
   * @param {() => number} [options.now] - injectable clock (ms epoch).
   * @param {number} [options.maxAgeDays] - archive retention in days.
   * @param {number} [options.maxPendingWrites] - maximum queued append tasks.
   * @param {number} [options.maxLiveBytes] - cap for the current day's log.
   ...
   * @param {(line: string) => void} [options.onMigrate] - called for each
   *   legacy audit file folded into the log directory at open().
  ...
  /**
   * Startup pass: fold any legacy audit files (written at the credential-dir
   * root by releases before 0.5.1) into this log directory, tighten the live
   * file's mode (appendFile's 0600 only applies at creation — a file that
   * predates this plugin version or was restored from a backup with looser
   * permissions is corrected here) and prune expired archives even if no
   * auth event arrives. Never rejects; failures go to onError.
   */
  open() {
  ...
  /**
   * Fold pre-0.5.1 audit files from the credential-dir ROOT (the location
   * the writer used before the log/ subdirectory existed — 0.5.1 moved the
   * write target without migrating what was already on disk) into this log
   * directory, so the audit trail never silently splits across two
   * locations after an upgrade.
   *
   * The source directory is the explicit `legacyDir` (defaulting to `dir`'s
   * parent, which is the credential dir for the default `dir`) rather than a
   * hidden `join(dir, '..')`, so an injected `dir` cannot silently aim the
   * migration at an unrelated tree.
   *
   * Rules, all failure-isolated (a migration problem must not block the
   * writer):
   * - a root `audit.log` with no `log/audit.log` yet becomes the live file
   *   (its content day is stamped from mtime at the next rotation);
   * - a root `audit.log` alongside an existing `log/audit.log` is a stale
   *   orphan: it is archived under its content day (no-clobber suffix
   *   bumps, mirroring #rollTo);
   * - root `audit.log.<date>` archives are moved as-is, no-clobber (a
   *   same-named file already in log/ means the content is already there —
   *   the root copy is left untouched rather than deleted);
   * - anything else in the root directory is left alone.
   *
   * Retention still applies to what is folded: an archive older than
   * `maxAgeDays` (and a legacy live file whose mtime day is that old, once
   * the first append rotates it) is pruned in this same startup pass, exactly
   * as it would have been had it already lived here.
   */
  async #migrateLegacyFiles() {
    const legacyDir = this.legacyDir
    ...
    try {
      ...
      this.#reportMigrationFailure(err, 1)
      return
    }
    let failed = 0
    let firstError
    for (const name of names) {
      if (name !== AUDIT_LOG_NAME && !/^audit\.log\.\d{4}-\d{2}-\d{2}(?:-\d+)?$/.test(name)) continue
      const src = join(legacyDir, name)
      let stamp
      try {
        const st = await stat(src)
        if (!st.isFile()) continue
        stamp = dayString(st.mtimeMs)
      } catch (err) {
        if (err.code === 'ENOENT') continue // vanished mid-scan
        failed += 1
        firstError ??= err
        continue
      }
      let dest
      try {
        if (name === AUDIT_LOG_NAME && !existsSync(join(this.dir, AUDIT_LOG_NAME))) {
          dest = join(this.dir, AUDIT_LOG_NAME) // becomes the live file
        } else if (name === AUDIT_LOG_NAME) {
          // Stale root live copy alongside an existing log/ live file: its
          // content predates the new live file, so archive it under its
          // content day (suffix-bumped, mirroring #rollTo).
          dest = join(this.dir, `${AUDIT_LOG_NAME}.${stamp}`)
          for (let n = 2; ; n++) {
            if (!existsSync(dest)) break
            if (n > MAX_SUFFIX_BUMP) throw new Error(`no collision-free archive name for ${name}`)
            dest = join(this.dir, `${AUDIT_LOG_NAME}.${stamp}-${n}`)
          }
        } else {
          // Rotated archive: keep its name; a same-named twin already in
          // log/ means the content is already preserved (link fails EEXIST
          // and the root copy is left untouched rather than deleted).
          dest = join(this.dir, name)
        }
        await this.#link(src, dest)
        await chmod(dest, 0o600)
        await unlink(src)
        this.onMigrate?.(`${src} -> ${dest}`)
      } catch (err) {
        // No-clobber collision on an archive whose twin already lives in
        // log/: the content is already preserved — leave the root copy in
        // place rather than delete data.
        if (err.code === 'EEXIST') continue
        failed += 1
        firstError ??= err
      }
    }
    if (failed > 0) this.#reportMigrationFailure(firstError, failed)
  }
  ...
  async #enqueue(fn) {
    if (this.pendingWrites >= this.maxPendingWrites) {
      this.#fail(new Error('audit log queue full; event dropped'))
      return
    }
  ...
  async #append(event) {
    ...
    const live = join(this.dir, AUDIT_LOG_NAME)
    let size = 0
    try { size = (await stat(live)).size } catch (err) {
      if (err.code !== 'ENOENT') throw err
    }
    if (size + Buffer.byteLength(line) > this.maxLiveBytes) {
      this.#fail(new Error('audit log size limit reached; event dropped'))
      return
    }
    await appendFile(live, line, { mode: 0o600 })
  ...
  /**
   * Report a startup-migration failure WITHOUT touching the append failure
   * streak. A legacy file that cannot be folded is worth a warning, but it
   * must not make the next successful append log a spurious "recovered"
   * (the migration problem itself is still there). Called at most once per
   * open(), carrying how many files were affected.
   */
  #reportMigrationFailure(err, count) {
    const error = err instanceof Error ? err : new Error(String(err))
    error.migrationFailures = count
    try {
      this.onError(error)
    } catch {
      // An onError that throws must not break the auth flow either.
