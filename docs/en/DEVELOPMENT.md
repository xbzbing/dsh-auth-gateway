...
├── lib/store.js      # Password storage (async scrypt hash, $DSH_HOME/auth-gateway/password.json)
...
├── lib/version.js    # Own version/repository (package.json) + SemVer-subset comparison
├── lib/update-check.js # New-version check (the only outbound request; TTL-cached, never throws)
...
├── scripts/uninstall.mjs     # dsh-auth-gateway-uninstall: deletes the whole auth-gateway/ (bilingual output)
...
- **Client trust timing**: an index script flips `isLoopback` while connection registers, before Settings makes its persistence decision; this is the project's documented security exception;
- **Settings nav order**: a `settings.section` registration's `order` must be strictly greater than every official section; `tests/client-contract.test.mjs` asserts this invariant;
- **The one outbound request, off by default**: `lib/update-check.js` queries the public npm registry for the `latest` tag behind the panel's "check for updates" action (the version number and repository link come from the local `package.json` and need no network). Automatic checks are **off** by default (`updateCheck: false`), so a fresh install never calls out; exactly two things trigger a request — a user pressing the button (`GET /login-api/version?refresh=1`), or the operator enabling `updateCheck` so opening the panel checks once. The constraints are hard: **never on the auth or panel critical path**, results AND failures are cached in memory (6h success / 15min failure, plus a 5s floor on repeated manual clicks), it never throws (a failure degrades to "cannot check", never to "up to date"), and an expired result is not presented as a verdict (`lastKnown()` reports only a cache entry within its TTL). See SECURITY.md;
- **Bilingual pages**: `lib/locale.js` reads `locale.preference` from the official `settings` service, then falls back to the request's `Accept-Language` and zh; error messages are maintained in one place, `lib/errors.js` (login failures return the single `invalid-credentials` code to prevent credential enumeration);
- **Login audit**: login success/failure/logout/password change and OTP enable/disable go through the `gateway.onAuthEvent` callback to the audit log (`ctx.logger.info`, only event kind + IP + reason — never credentials) and, together with brute-force alerts (`onSecurityEvent`), are appended by lib/audit-log.js to `$DSH_HOME/auth-gateway/log/audit.log` (JSONL, daily rotation, 90-day retention; write failures only warn and never touch the auth flow) — the current dsh runtime keeps `ctx.logger` records in an in-memory buffer only, so this file is the single persistent audit record;
...
- `0.4.0`: OTP secret encrypted at rest with AES-256-GCM (master key from `DSH_AUTH_GATEWAY_MASTER_KEY` or an auto-generated `otp-master.key`);
