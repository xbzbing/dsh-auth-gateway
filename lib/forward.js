 ...
 * canonical hop-by-hop names, and the framing headers (mirroring the
 * request side — the piped body stream is already decoded, so the
 * upstream's `content-length`/`transfer-encoding` describe a framing that
 * no longer applies, and Node re-frames the response for this hop, which
 * also turns a truncated upstream body into an exposed framing error
 * instead of a silently short response). End-to-end response headers
 * (`content-type`, `content-encoding`, `vary`, `date`, ...) survive
 * untouched.
 *
