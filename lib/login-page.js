...
const page = (locale, basePath) => makePage(STYLE, buildScript(locale, basePath))
...
wire('auth', __basePath + '/login/auth', authPayload, (data) => {
  if (data && data.otpRequired) {
    location.href = __basePath + '/otp/verify'
  } else {
...
export function loginPageHtml({ mode, otpEnabled = false, digits = 6, otpRequired = false, locale = 'zh', basePath = '/' }) {
