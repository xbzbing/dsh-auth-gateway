...
- **修改密码入口**：打开 dsh 的**设置面板**，左侧导航栏会出现"修改密码"入口（插件注入，与"通用/模型/插件"并列），点击直达改密表单；直接访问 `/login`（已登录时同样显示改密表单）。
...
- **暴力破解防护有限**：有按来源的失败锁定（默认 5 次/5 分钟，可配置），但无全局限速、无分布式/换 IP 防护。
- **内存会话**：dsh 重启后全员下线。
- 后续工作：TLS、多用户、SPA 内嵌设置项。
