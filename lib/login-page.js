...
import { baseStyle, scriptHead, strengthErrorScript, makePage, normalizeBasePath } from './page-shell.js'
 ...
 */
/**
 * Source of the login page's post-login session probe, embedded verbatim
 * into every login page. Exported so tests can execute the EXACT code the
 * browser will run (instantiate with `new Function('ERRORS', 'fetch', SRC +
 * '; return probeSession')`) instead of testing a drifted copy — the same
 * rationale as tests/patch-ports.test.mjs evaluating the patch's `!!js`
 * sources.
 *
 * Behavior: fetch `<basePath>/login-api/session` — 200 → the login proceeds;
 * 401 → the browser refused to keep the session cookie (an HTTPS entry's
 * Secure cookie blocks the plaintext entry, see docs/zh/DEPLOYMENT.md
 * 「混合 HTTPS/HTTP 入口」), so surface ERRORS['session-not-kept'] and stay on
 * the page; any other status or a network error → keep the legacy behavior
 * (a probe hiccup must never block a working login).
 *
 * The ERRORS['session-not-kept'] key is a hard dependency of this snippet:
 * it must exist in lib/errors.js AND be listed in ERROR_KEYS below (the
 * page dictionary is key-filtered by errorsFor). A miss renders an empty
 * error message — pinned by the page-body regression assertion in
 * tests/gateway.test.mjs.
 */
export const PROBE_SESSION_SOURCE = `
async function probeSession(err, btn, basePath) {
  try {
    const res = await fetch(basePath + '/login-api/session', { headers: { accept: 'application/json' } })
    if (res.status === 200) return true
    if (res.status === 401) {
      err.textContent = ERRORS['session-not-kept']
      btn.disabled = false
      return false
    }
  } catch { /* probe hiccup: keep the legacy behavior */ }
  return true
}
`
...
function wire(formId, path, build, onOk, opts) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    // Forms that carry an OTP field must have a full code.
...
const page = (locale, basePath) => makePage(STYLE, buildScript(locale, basePath), locale === 'en' ? 'en' : 'zh-CN', basePath)
...
wire('auth', __basePath + '/login/auth', authPayload, async () => {
  // Login success: verify the session cookie actually took before entering
  // the app (see probeSession above), otherwise the user would land back on
  // this page forever with a "correct" password.
  const kept = await probeSession(
    document.getElementById('error'),
    document.querySelector('#auth button[type=submit]'),
    __basePath,
  )
  if (kept) location.href = __basePath + '/'
}, { otpDigits: ${digits} })
