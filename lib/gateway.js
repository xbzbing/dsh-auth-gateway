...
export class LoginGateway {
  ...
  constructor({ listenHost, listenPort, upstreamHost, upstreamPort, basePath = '/', policy = {}, otp = {}, cookieSecure = 'auto', cookieSecureOverride, cookieSecureOverrideStore, upstreamSecretReader, versionInfo, updateChecker, updateCheckAuto = false }) {
    ...
    // panel-owned value, takes precedence over the composition above.
    this.cookieSecureOverride = cookieSecureOverride === true || cookieSecureOverride === false || cookieSecureOverride === 'auto'
      ? cookieSecureOverride
      : null
    /**
     * Persistence half of the override, wired by the plugin entry
     * (ctx.credentials record 'dsh-auth-gateway/cookie-secure'); absent →
     * the POST route answers storage-unavailable and the panel stays
     * read-only.
     * @type {{read: () => Promise<'auto'|true|false|null>, write: (mode: 'auto'|true|false) => Promise<void>, clear: () => Promise<void>} | undefined}
     */
    this.cookieSecureOverrideStore = cookieSecureOverrideStore
  ...
  async #route(req, res) {
    ...
    if (req.method === 'GET' && pathname === '/login-api/session') {
      return this.#handleSessionProbe(req, res)
    }
    if (req.method === 'POST' && pathname === '/login-api/cookie-secure') {
      return this.#handleSetCookieSecure(req, res)
    }
    ...
    if (req.method === 'GET' && pathname === '/login-api/version') {
      return this.#handleGetVersion(req, res)
    }
    ...
    this.#json(res, 200, {
      ...
      config: {
        'dsh-auth-gateway': {
          ...
          cookieSecure: this.#cookieSecureMode(),
          cookieSecureSource: this.cookieSecureOverride === null || this.cookieSecureOverride === undefined
            ? 'deployment'
            : 'panel',
        },
      },
 ...
 * redirect target anyway. */
  #handleSessionProbe(req, res) {
    const token = tokenFromCookieHeader(req.headers.cookie)
    if (token === undefined || !this.sessions.isValid(token)) {
   ...
   */
  async #handleSetCookieSecure(req, res) {
    const token = this.#verifiedTokenOr401(req, res)
    if (token === undefined) return

    if (this.cookieSecureOverrideStore === undefined) {
      return this.#json(res, 400, { ok: false, error: 'storage-unavailable' })
    }
    ...
    if (body !== null && typeof body === 'object' && body.reset === true) {
      ...
      if (mode !== 'auto' && mode !== true && mode !== false) {
        return this.#json(res, 400, { ok: false, error: 'invalid-mode' })
      }
    }

    try {
      if (reset) {
        await this.cookieSecureOverrideStore.clear()
        this.cookieSecureOverride = null
        this.#authEvent('cookie-secure-change', req.socket.remoteAddress ?? 'unknown', 'reset')
      ...
      // boot.
      await this.cookieSecureOverrideStore.write(mode)
      this.cookieSecureOverride = mode
      this.#authEvent('cookie-secure-change', req.socket.remoteAddress ?? 'unknown', String(mode))
      return this.#json(res, 200, {
        ok: true,
        cookieSecure: mode,
      ...
      return this.#json(res, 500, { ok: false, error: 'storage-failed' })
    }
  }
  ...
  async #handleGetVersion(req, res) {
    ...
    if (this.updateChecker !== null) {
      try {
        if (refresh) update = await this.updateChecker.check({ force: true })
        else if (this.updateCheckAuto) update = await this.updateChecker.check()
        else update = this.updateChecker.lastKnown()
      } catch (err) {
        this.onError?.(err)
      }
    }
    this.#json(res, 200, {
      ...
      repository: this.versionInfo.repository,
      update,
    })
  }
  ...
  /**
   * What the session cookie's Secure policy currently is: the panel-owned
   * override when one exists, otherwise the composition value.
   */
  #cookieSecureMode() {
    if (this.cookieSecureOverride !== null && this.cookieSecureOverride !== undefined) {
      return this.cookieSecureOverride
    }
   ...
   * pinned true/false by the effective policy (override or config), else
   * auto-followed from the request's transport (see lib/auth.js
   * requestIsSecure).
   */
  #secureCookie(req) {
    const mode = this.#cookieSecureMode()
  ...
  #secureCookieOrWarn(req) {
    ...
    if (this.#cookieSecureMode() === true && !requestIsSecure(req)) {
