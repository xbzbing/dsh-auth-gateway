...
const ERROR_KEYS = [
  ...
  // Step 1 posts to /otp/enable and /otp/verify-setup: the codes those
  // endpoints answer — a wrong code (the page's highest-frequency error),
  // the per-address rate cap and the already-enabled conflict — must resolve
  // through the dictionary, never surface as raw machine codes.
  'invalid-otp', 'rate-limited', 'otp-already-enabled',
]
...
(async () => {
  ...
  try {
    ...
    // post() throws on a non-ok response with the dictionary-mapped message,
    // so a resolved data here is always ok:true.
