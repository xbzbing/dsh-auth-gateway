...
- **The one outbound request**: `lib/update-check.js` queries the public npm registry for the `latest` tag behind the panel's new-version notice (the version number and repository link come from the local `package.json` and need no network). The constraints are hard: **never on the auth or panel critical path**, results AND failures are cached in memory (6h success / 15min failure, so an unreachable registry is not hammered), it never throws (a failure degrades to "cannot check", never to "up to date"), and `updateCheck: false` turns it off entirely. See SECURITY.md;
