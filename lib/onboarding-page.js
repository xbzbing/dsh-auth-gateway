 ...
 * @param {object} options - `{ otpBindingAvailable }`: whether the optional
 *   OTP-binding step is shown (false once 2FA is already active);
 *   `{ locale }`: 'zh' (default) or 'en'.
 */
export function onboardingPageHtml({ otpBindingAvailable = true, locale = 'zh' } = {}) {
  const str = STR[locale] || STR.zh
  const otpStep = otpBindingAvailable ? `
<div class="step">
  <h3>${str.step2H3}</h3>
  <p>${str.step2POn}</p>
