...
export const ERRORS = {
  zh: {
    ...
    'session-not-kept': '登录未生效：浏览器未能保存会话 Cookie。若此前用 HTTPS 登录过，请改用 HTTPS 访问，或在 HTTPS 入口退出登录后重试；否则请刷新页面重试。',
  ...
  en: {
    ...
    'session-not-kept': 'Sign-in did not take effect: the browser did not keep the session cookie. If you previously signed in over HTTPS, use HTTPS instead, or sign out on the HTTPS entry and retry; otherwise refresh and try again.',
