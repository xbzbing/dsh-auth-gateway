...
import { tokenFromCookieHeader, requestIsSecure, sessionCookie } from './auth.js'
...
/** Secure decision for a cookie under a given policy mode: pinned true/false,
 * else auto-followed from the request transport — the mirror of the
 * gateway's own decision (lib/gateway.js #secureCookie). */
function secureFor(mode, req) {
  ...
  async function handleSetCookieSecure(req, res) {
    ...
    try {
      if (reset) {
        ...
        }, sessionCookie(token, secureFor(cookieSecureMode(), req)))
      }
      ...
      }, sessionCookie(token, secureFor(mode, req)))
    } catch (err) {
