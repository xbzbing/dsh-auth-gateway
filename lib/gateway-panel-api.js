...
export function createPanelApi(gateway, priv, { cookieSecureOverride, cookieSecureOverrideStore } = {}) {
  ...
  let override = cookieSecureOverride === true || cookieSecureOverride === false || cookieSecureOverride === 'auto'
    ? cookieSecureOverride
    : null

  /** Effective Secure policy: the panel override when one exists, else the composition value. */
  const cookieSecureMode = () => (override === null ? gateway.cookieSecure : override)

  /** Where the effective policy comes from, for the panel card. */
  const cookieSecureSource = () => (override === null ? 'deployment' : 'panel')
  ...
  async function handleSetCookieSecure(req, res) {
    ...
    try {
      if (reset) {
        ...
        override = null
        authEvent('cookie-secure-change', req.socket.remoteAddress ?? 'unknown', 'reset')
        return json(res, 200, {
          ok: true,
          cookieSecure: cookieSecureMode(),
      ...
      override = mode
      authEvent('cookie-secure-change', req.socket.remoteAddress ?? 'unknown', String(mode))
      return json(res, 200, {
        ok: true,
        cookieSecure: mode,
