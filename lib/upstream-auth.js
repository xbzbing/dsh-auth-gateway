/**
 * Upstream browser-auth cookie.
 *
 * dsh's internal webserver arms `BrowserAuth`: the index fallback and every
 * /api route demand either the process launch token (a one-shot `?token=`
 * exchange) or a persistent, authority-bound, HMAC-signed cookie. The browser
 * behind this gateway holds only the gateway's session cookie — it can never
 * earn the upstream cookie through that exchange — so forwarded requests would
 * all 401 with "dsh web authentication required; reopen the URL printed by dsh
 * web".
 *
 ...
 *   to "never known" — no cookie is minted.
 * @returns {{ cookieHeader: () => string | undefined }} call per request;
 *   undefined means the credential record is not available yet.
 */
export function createUpstreamCookieMinter(authority, readSecret) {
  ...
  const cookieHeader = () => {
    ...
    if (current === undefined) {
      // No secret known (the record was just revoked): drop any live cookie —
      // attaching a stale one could only leak it verbatim.
 ...
 *
 * A missing record or a failed read yields undefined until the credential
 * record becomes available. Never throws; read failures go through onError
...
export function createCachedSecretReader({
  ...
  return {
    ...
    warm: () => {
      ...
      // the in-flight read). A failed read resolves too — the retry probe
      // keeps looking until the record (and its secret) exists.
