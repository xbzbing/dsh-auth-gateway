...
export class LoginGateway {
   ...
   */
  constructor({ listenHost, listenPort, upstreamHost, upstreamPort, basePath = '/', policy = {}, otp = {}, cookieSecure = 'auto', upstreamSecretReader, versionInfo, updateChecker, updateCheckAuto = false }) {
    this.listenHost = listenHost
    this.listenPort = listenPort
    this.upstreamHost = upstreamHost
    this.upstreamPort = upstreamPort
    // Cookie Secure policy ('auto' | true | false); normalized by the plugin
    // Config validator, but an embedder can pass anything — treat only the
    // booleans as pinning, everything else as auto-follow.
  ...
  async #handleAuth(req, res) {
    ...
    if (otpRequired && otpEnabled) {
      ...
      return this.#json(res, 200, { ok: true }, sessionCookie(token, this.#secureCookie(req)))
    }
    ...
    this.#json(res, 200, { ok: true }, sessionCookie(token, this.#secureCookie(req)))
  }
    ...
    this.#json(res, 200, {
      ...
      config: {
        'dsh-auth-gateway': {
          ...
          otpStatus,
          // Cookie Secure policy for the panel's status card: 'auto', true
          // or false (lib/config.js). Read-only — the composition (bundle /
          // profile patch) owns it, exactly like every other config field.
   ...
   * pinned true/false by the `cookieSecure` config, else auto-followed from
 ...
 */
export function createGateway(config, { upstreamSecretReader, versionInfo, updateChecker, updateCheckAuto } = {}) {
  ...
  return new LoginGateway({ listenHost, listenPort, upstreamHost, upstreamPort, basePath, policy, otp, cookieSecure: config?.cookieSecure ?? 'auto', upstreamSecretReader, versionInfo, updateChecker, updateCheckAuto })
}
