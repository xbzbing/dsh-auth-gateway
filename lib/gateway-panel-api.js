...
import { tokenFromCookieHeader, requestIsSecure, secureForPolicy, sessionCookie } from './auth.js'
...
export function createPanelApi(gateway, priv, { cookieSecureOverride, cookieSecureOverrideStore } = {}) {
  ...
  // Named panelOverride — `override` would collide with the gateway class's
  // own option key of the same spelling in this module's call sites.
   ...
   * new policy (same token, fresh Max-Age), so the next request — not the
   * next login — carries the new Secure attribute. The browser accepts the
   * overwrite only from a secure origin: a plaintext entry cannot replace a
   * stored Secure cookie, and the panel warns for exactly that case (the
   * gateway cannot see cookie attributes and has no way to detect the
   * refused overwrite server-side).
   */
  async function handleSetCookieSecure(req, res) {
    ...
    // Arming Secure from a plaintext entry is a self-lockout: the browser
    // refuses to store the Secure cookie, so every subsequent login
    // "succeeds" but holds no session. The panel already blocks this click;
    // this check is the curl-proof server-side half.
    if (mode === true && !requestIsSecure(req)) {
      return json(res, 400, { ok: false, error: 'force-secure-requires-tls' })
    }
    ...
    try {
      ...
      // One answer for both paths: report the now-effective policy and
      // REISSUE the caller's own session cookie under it, so the new Secure
      // attribute applies to THIS browser on the very next request instead of
      // at its next login. The browser accepts the overwrite only from a
      // SECURE origin — a plaintext entry cannot replace a stored Secure
      // cookie (the panel warns for that case; the gateway cannot see cookie
      // attributes, so it would never detect the refused overwrite).
      const effective = cookieSecureMode()
      return json(res, 200, {
        ok: true,
        cookieSecure: effective,
        cookieSecureSource: cookieSecureSource(),
      }, sessionCookie(token, secureForPolicy(effective, req)))
    } catch (err) {
