...
TOTP secret 是第二因素的根密钥：拿到它就能生成任意有效验证码。为阻止 `$DSH_HOME` 文件泄露直接交出该密钥，`otp-store.js` 在写入 `otp.json` 前用 `lib/otp-crypto.js` 将其以 **AES-256-GCM** 密封（格式 `v1.<iv>.<tag>.<cipher>`，均 hex），读取时再用主密钥解密。未密封的记录被视为损坏并拒绝读取（`otp-secret-corrupted`）。
...
`x-forwarded-for` 不参与来源判定（防止伪造）；锁触发与限流耗尽时输出 `ctx.logger.warn` 并广播 `dsh-auth-gateway/brute-force` 事件（`{kind: 'lockout'|'global-rate-limit'|'otp-rate-limit', ...}`，JSON 负载，每个锁定/窗口一次）。认证事件与暴力破解告警同时**持久化落盘** `$DSH_HOME/auth-gateway/log/audit.log`（JSONL，每行一个 `{ts, kind, ip, reason?, ...}` 对象，文件 0600）：活跃文件按本地日历日轮转为 `audit.log.<YYYY-MM-DD>`，归档保留 90 天后删除；启动时收紧既有文件权限至 0600 并清理过期归档，优雅停机时等待在途写入落盘（仅硬崩溃可能丢失正在写入的最后一行）；写失败降级为告警日志且自动去重（首次即时上报，持续失败每 5 分钟提醒一次并附抑制计数，恢复时记录 info），绝不影响认证流程。
...
- **密钥经官方通道读取**：通过注入的 `credentials` 服务读取官方 record `client-connection/browser-session`（与 dsh 自身 BrowserAuth 读写的是同一条 `ctx.credentials.readRecord` 通道；`kind: grant`、payload `version: 1`、32 字节密钥，与 dsh 的 `storedSecret` 同一校验口径）。不依赖 `.credentials.yaml` 的私有文件形态；record 不存在（dsh ≤ 0.1.1）时静默退化为逐字转发，行为与旧版一致。缓存语义：**从未读到过密钥时**（全新部署，record 由 dsh 的 Connection 激活时创建、可能晚于网关启动）按 2s 快速重试并在后台轮询，record 一出现转发即携带铸出的 cookie；**读到过密钥后**按 60s 异步刷新跟随轮换。转发请求只走同步快路径。
...
- **明文 HTTP（传输层）**：密码与 Cookie 在网络中明文传输。这是传输层问题——本插件的会话 Cookie 支持 `Secure` 属性（配置项 `cookieSecure`，默认 `auto`）：当连接经 TLS（反向代理终结并透传 `X-Forwarded-Proto: https`）时自动附加，浏览器从此只经加密链路回传 Cookie；置 `true` 可强制附加（反代 TLS 但未透传协议头时使用）——注意纯 HTTP 下强制开启会使浏览器拒绝保存 Cookie、登录立即失败，这是**显式的失败而非静默降级**（网关还会在每次此类登录时通过 dsh 控制台告警，提示前置 TLS 或改回 `auto`）；`false` 显式关闭。**面板可运行时修改**（「认证设置 → Cookie 安全」）：覆盖值持久化于凭据记录并优先于部署配置，「恢复为部署配置」撤销后回到 patch 值——部署者无需改 patch、重启即可在反代上线 TLS 时立即武装 Secure。切换**即时生效**：保存成功后网关会以新策略重发当前会话 Cookie（同一 token、属性即改），下一次请求就已按新模式走——但浏览器只允许**安全来源**改写已存在的 Secure Cookie：若从明文 HTTP 入口选择「关闭」，而浏览器仍存有此前经 HTTPS 留下的 Secure Cookie，该覆盖会被浏览器拒绝且服务端无从感知，故面板会提示改用 HTTPS 入口重新执行或手动清除站点 Cookie；反过来，在明文 HTTP 入口选择「强制开启」会被**面板与服务端一并拒绝**并说明原因（强制 Secure 会让该入口的登录立即失效，服务端同样以 `force-secure-requires-tls` 拒绝直连请求）。Secure 只解决「Cookie 不被明文链路回传」，**不加密请求本身**——要真正消除明文暴露仍须部署 TLS（反向代理或证书，见 DEPLOYMENT.md）；
...
- **面板可改部署级策略（cookieSecure）**：「认证设置」的 Cookie 安全卡片允许**任意已认证会话**修改 `cookieSecure`（含改为 `false` 关闭传输保护）——这是有意的运行时便利，但意味着多用户部署中任一用户可影响全体会话的传输策略；单用户内网部署无实际影响。每次修改都经审计（`cookie-secure-change`）；若多用户部署需要更严格门槛，应改由部署配置锁定（面板覆盖可被「恢复为部署配置」之外的部署约束取代，见 DEPLOYMENT.md）。
- **对外网络请求（新版本检查）**：这是本插件**唯一**的对外连接，且**默认关闭**——全新安装、以及面板每次打开，都不会联网。只有两种情况会触发：① 已登录用户在「认证设置 → 关于」卡片上**点击「检查更新」**（该用户本人的明确意图，也是默认配置下唯一的触发方式）；② 部署方把 `updateCheck` 设为 `true`，此时面板打开会**自动**检查一次。请求是对公共 npm registry 的一个 GET（`https://registry.npmjs.org/dsh-auth-gateway/latest`，超时 3s），不含任何凭据、会话 cookie 或实例标识，但**你部署的出口 IP 对 registry 可见**。结果与失败都缓存在内存（成功 6h、失败 15min），手动重复点击另有 5s 下限，避免打成请求洪流；registry 不可达时面板显示「暂时无法检查更新」而非报错，认证与转发路径完全不受影响。当前版本号与仓库链接来自本机 `package.json`，**不联网**。
