...
export class LoginGateway {
   ...
   *   (lib/update-check.js); absent → the panel reports update checks as
   *   disabled and makes no outbound request.
   */
  constructor({ listenHost, listenPort, upstreamHost, upstreamPort, basePath = '/', policy = {}, otp = {}, upstreamSecretReader, versionInfo, updateChecker }) {
   ...
   * new-version verdict for the settings panel.
   *
   * The registry lookup is awaited but must never be able to break the
   * response: `status()` is contracted never to throw (lib/update-check.js),
   * and the catch here is the second line of defence — a panel that cannot
   * hear about a new release still shows the version and the link.
   */
  async #handleGetVersion(req, res) {
    const token = this.#verifiedTokenOr401(req, res)
    if (token === undefined) return

    let update = { enabled: false, latest: null, updateAvailable: null, checkedAt: null, error: null }
    if (this.updateChecker !== null) {
      try {
        update = await this.updateChecker.status()
      } catch (err) {
        this.onError?.(err)
      }
    }
 ...
 */
export function createGateway(config, { upstreamSecretReader, versionInfo, updateChecker } = {}) {
  ...
  return new LoginGateway({ listenHost, listenPort, upstreamHost, upstreamPort, basePath, policy, otp, upstreamSecretReader, versionInfo, updateChecker })
}
