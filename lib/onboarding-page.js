 ...
 */
const page = (locale, extraScript) => makePage(STYLE, scriptHead(errorsFor(locale, ERROR_KEYS, ERROR_OVERRIDES)) + (extraScript || ''))
 ...
 */
export function onboardingPageHtml({ locale = 'zh' } = {}) {
...
(async () => {
  ...
  try {
    const data = await post('/otp/enable', {});
...
document.getElementById('verify-form').addEventListener('submit', async (e) => {
  ...
  btn.disabled = true;
  try {
    const data = await post('/otp/verify-setup', { otp: code });
    ...
    document.getElementById('otp-done-btn').onclick = () => { location.href = data.next || '/onboarding/password'; };
    err.textContent = '';
  } catch (ex) { err.textContent = ex.message; btn.disabled = false; }
});
`)(str.otpTitle, str.otpSub, `
  ...
  <div class="otp-grid">
    ...
    <div class="otp-right">
      <div class="step">
        ...
        <button class="secondary" type="button" onclick="location.href='/onboarding/password'" style="margin-top:8px;width:100%;">${str.btnSkip}</button>
      </div>
    </div>
  </div>
</div>
 ...
 * @param {object} options - `{ locale }`: 'zh' (default) or 'en'.
 */
export function onboardingPasswordPageHtml({ locale = 'zh' } = {}) {
  const str = STR[locale] || STR.zh
...
`)(str.passwordTitle, str.passwordSub, `
<form id="change-form">
  <div class="pw-step">
    <div class="step">
