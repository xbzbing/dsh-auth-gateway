...
- **OTP 未激活时的启用权限（DoS 面）**：OTP 尚未启用时，`/otp/enable` 与 `/otp/verify-setup` 仅要求任意有效会话——密码泄露且 `otpRequired: false` 场景下，攻击者可用泄露的密码登录后绑定自己的认证器，锁死真实用户登录。这不构成凭据窃取，主要是 DoS 面；后续方向为启用时要求密码重验证；
