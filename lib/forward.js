...
export function createForwarder({ upstreamHost, upstreamPort, upstreamSecretReader, onError }) {
  ...
  function forward(req, res, forwardPath, { upstreamAuth = true } = {}) {
    ...
    // Let Node recompute request framing from the piped body. A copied
    // content-length can disagree with the decoded body stream (e.g. a client
    // that sent both Content-Length and Transfer-Encoding — the CL.TE
    // smuggling shape), and a copied transfer-encoding would re-frame the
    // already-decoded stream; dropping both closes the ambiguity.
