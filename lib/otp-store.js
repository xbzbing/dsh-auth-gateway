 ...
 *
 * Manages TOTP secrets and backup codes in $DSH_HOME/auth-gateway/otp.json.
...
import { seal, unseal, isSealed, OTPCryptoError, OTP_CRYPTO_ERROR } from './otp-crypto.js'

/** Absolute path of the OTP record file. */
export function otpFilePath() {
  return join(credentialDir(), 'otp.json')
}
...
export function getOTPSecret() {
  ...
  if (!isSealed(record.secret)) {
    throw new OTPCryptoError(OTP_CRYPTO_ERROR.SECRET_CORRUPTED, 'dsh-auth-gateway: OTP secret is not sealed')
  }
  try {
 ...
 *
 * Single-use is a security invariant ("每个备份代码只能使用一次"): the check
 * spans an async scrypt gap, so two concurrent submissions of the SAME code
 * (a stolen code raced against the legitimate holder, or a double-click) must
 * not both land `used: false` and mint two sessions. Code verification is
 * therefore serialized through one per-process mutex — the reads, scrypt
 * checks and the used-marking all happen inside the same critical section.
 * The lock is a simple promise chain: O(1) per submission, never held by a
 * full lockout window.
 * @param {string} code - Backup code to verify.
 * @returns {Promise<boolean>} Whether the code was valid and unused.
 */
let backupCodeChain = Promise.resolve()
export async function verifyAndUseBackupCode(code) {
  let release
  const acquire = new Promise((resolve) => { release = resolve })
  const prev = backupCodeChain
  backupCodeChain = backupCodeChain.then(() => acquire)
  await prev
  try {
    const record = readRecord()
