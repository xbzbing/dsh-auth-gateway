...
import { baseStyle, scriptHead, makePage, normalizeBasePath } from './page-shell.js'
...
const ERROR_KEYS = [
  ...
  // Edge paths of /otp/verify-setup and the verify page: the staged temp
  // secret expired (restart / long-idle session) and the stored secret
  // missing — both documented dictionary entries, not raw codes.
  'setup-expired', 'otp-secret-missing',
]
...
const page = (locale, basePath) => makePage(STYLE, scriptHead(errorsFor(locale, ERROR_KEYS, ERROR_OVERRIDES)) + `\nconst __basePath = ${JSON.stringify(normalizeBasePath(basePath))};\n`, locale === 'en' ? 'en' : 'zh-CN', basePath)

/**
 * HTML-escape a value interpolated into page markup. The secret and backup
 * codes come from generator charsets that today contain no HTML-special
 * characters, but the onboarding page assigns the same values via textContent
 * — keep this page equally hostile to a future charset change (defense in
 * depth, not a fix for a reachable injection).
 */
...
const useBackupEl = document.getElementById('use-backup');
if (useBackupEl) {
  useBackupEl.addEventListener('click', () => {
...
document.getElementById('backup-form').addEventListener('submit', async (e) => {
  e.preventDefault();
  const err = document.getElementById('error');
  // The backup form's OWN submit button — a document-wide query would pick
  // the OTP form's button and leave the backup one clickable mid-flight.
...
<div class="step">
  <h3>${str.step2H3}</h3>
  <div class="secret-box" id="secret-display">${escapeHtml(secret)}</div>
</div>
...
<div class="backup-codes">
  <h3>${str.backupH3}</h3>
  <pre id="backup-codes">${escapeHtml(codesText)}</pre>
