...
function wire(formId, path, build, onOk, opts) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    // Forms that carry an OTP field (login with 2FA) must have a full
    // 6-digit code before the button state changes.
    if (payload.otp !== undefined && payload.otp.length !== 6) {
      err.textContent = '请输入 6 位 OTP 验证码'
      return
    }
    btn.disabled = true
    try {
...
const SCRIPT_AUTH = `
wire('auth', '/login/auth', (f) => ({ password: f.password.value, otp: f.otp ? f.otp.value : undefined }), (data) => {
...
export function loginPageHtml({ mode, otpEnabled = false }) {
  if (mode === 'setup') {
...
<form id="auth">
  ...
  <input id="otp" type="text" pattern="[0-9]{6}" maxlength="6" inputmode="numeric" autocomplete="one-time-code" required placeholder="000000">
