...
// Post-login probe: a login whose Set-Cookie the browser refused to keep —
// the plain-HTTP entry colliding with an HTTPS entry's Secure cookie (see
// docs/zh/DEPLOYMENT.md「混合 HTTPS/HTTP 入口」) — would otherwise
// "succeed" and bounce straight back to /login with no explanation. Verify
// the cookie is actually alive before navigating away; a probe hiccup
// (timeout, proxy) never blocks a working login.
async function probeSession(err, btn) {
  try {
    const res = await fetch(__basePath + '/login-api/session', { headers: { accept: 'application/json' } })
