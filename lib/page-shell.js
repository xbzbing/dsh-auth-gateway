 ...
 * page-specific CSS rules and JS. It also owns the two helpers shared with
 * the gateway itself: the basePath normalization every page script needs and
 * the response writer the gateway serves those pages with.
 *
 * No user-controlled content is ever interpolated into these pages; the
 * script prologue additionally escapes anything inlined into <script> (see
 * scriptHead), so the boundary holds even if that invariant erodes.
 */
 ...
 *
 * The dictionary is inlined into a <script> element. Today every message is a
 * static author-maintained string, but the boundary is defensive by design:
 * `<`/`>` escape as \u003c/\u003e (so no message can ever close the script
 * tag) and U+2028/U+2029 escape as \u2028/\u2029 (otherwise valid JS line
 * separators break the string literal).
 ...
 * @param {string} [lang] - `lang` attribute for the document ('zh-CN' default;
 *   pages pass 'en' for English renders so assistive tech picks the right one).
 */
export function makePage(style, script, lang = 'zh-CN', basePath = '') {
  // The icon must ride the gateway's basePath ('' or '/dsh'): root-absolute
  // hrefs would bypass the sub-path prefix and miss the allowlisted route.
  const stripped = basePath.replace(/^\/+|\/+$/g, '')
  const iconHref = (stripped === '' ? '' : '/' + stripped) + '/favicon.svg'
...
<title>${title}</title>
<!-- Declare the icon so the browser never auto-requests /favicon.ico (which
     is not in the gateway's public-asset allowlist and would 404 noisily).
     /favicon.svg IS allowlisted and forwards to dsh's own icon. -->
...
/**
 * Normalize a gateway basePath for embedding in a page script: '/' and ''
 * both mean root (''), a sub-path keeps its '/prefix' form (the gateway
 * constructor already trims stray slashes, so only the root spelling needs
 * collapsing here).
...
/**
 * Serve a self-contained gateway page: 200, HTML, never cached. `no-store`
 * is part of the model — these pages render security state (login vs
 * change-password mode, onboarding progress) and must not be replayed from
 * a shared cache to another client.
 * @param {import('node:http').ServerResponse} res
 * @param {string} html - a fully built page.
 */
