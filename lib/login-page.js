...
const ERRORS = {
  ...
  'password-needs-case': '密码需同时包含大写和小写字母',
...
function strengthError(pw) {
  ...
  if (!/[a-z]/.test(pw) || !/[A-Z]/.test(pw)) return ERRORS['password-needs-case']
  return null
}
// strengthField names the payload field to strength-check; pass undefined for
// forms that must not be checked (login: the old password may predate the
// policy). The server remains the authority — this is early feedback only.
function wire(formId, path, build, onOk, strengthField) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (strengthField !== undefined) {
      const weak = strengthError(payload[strengthField])
...
const SCRIPT_SETUP = `
wire('setup', '/login/setup', (f) => ({ password: f.password.value }), undefined, 'password')
`
...
export function loginPageHtml({ mode }) {
  if (mode === 'setup') {
    return page('设置密码', '首次使用：请设置访问密码（至少 8 位，需同时包含大小写字母），之后每次打开都需要登录。', `
<form id="setup">
...
<form id="change">
  ...
  <input id="newPassword" type="password" autocomplete="new-password" required minlength="1">
