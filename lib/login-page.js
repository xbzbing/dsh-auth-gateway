...
function wire(formId, path, build, onOk, opts) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    // before the button state changes. digits comes from opts.otpDigits
    // (server config), defaulting to 6.
...
export function loginPageHtml({ mode, otpEnabled = false, digits = 6 }) {
  if (mode === 'setup') {
