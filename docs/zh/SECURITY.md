...
- **对外网络请求（新版本检查）**：这是本插件**唯一**的对外连接。设置面板打开「关于」卡片时，网关会向公共 npm registry 发一个 GET（`https://registry.npmjs.org/dsh-auth-gateway/latest`，超时 3s），用于判断是否有新版本；请求不含任何凭据、会话 cookie 或实例标识，你部署的出口 IP 对 registry 可见。结果与失败都缓存在内存（成功 6h、失败 15min），registry 不可达时面板显示「暂时无法检查更新」而非报错，认证与转发路径完全不受影响。如需完全离线，配置 `updateCheck: false`：面板仍显示当前版本与仓库链接（两者来自本机 `package.json`，不联网）。
